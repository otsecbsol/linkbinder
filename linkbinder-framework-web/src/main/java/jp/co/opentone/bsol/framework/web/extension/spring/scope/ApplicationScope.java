...
/**
 * JSF2.0の@ApplicationScopedと同様のスコープを実現する.
 * <p>
 * @author opentone
 */
public class ApplicationScope implements Scope {
    ...
    Map<String, Object> getMap() {
        ...
        if (c.getExternalContext() == null) {
            return null;
        }
        return c.getExternalContext().getApplicationMap();
    }
    ...
    public Object get(String name, ObjectFactory<?> objectFactory) {
        ...
        if (map != null) {
            if (map.containsKey(name)) {
                result = map.get(name);
                log.debug("Instance of bean '{}' exists in application scope.", name);
            } else {
                Object bean = objectFactory.getObject();
                log.debug("created instance of bean '{}' in application scope.", name);
                map.put(name, bean);
                result = bean;
            }
        }
