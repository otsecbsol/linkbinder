...
/**
 * {@link CopyCorresponSetupStrategy}のテストケース.
 * @author opentone
 */
public class CopyCorresponSetupStrategyTest extends AbstractCorresponSetupStrategyTestCase {

    private Correspon correspon;

    public CopyCorresponSetupStrategyTest() {
        super(CorresponEditMode.COPY);
    }
    ...
    public void testSetupAttachemtsTransfer() throws Exception {
        ...
        assertEquals(correspon.getDeadlineForReply(), actual.getDeadlineForReply());
        //  attachments
        //  引き継ぎ指定したファイルのみ反映されている
        assertEquals(2, actual.getAttachments().size());
